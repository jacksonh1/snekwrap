...
from Bio.PDB import PDBParser, PDBIO, MMCIFParser, PPBuilder, Structure  # type: ignore
...
def biopython_structure_to_string(structure: Structure.Structure) -> str:
...
def assign_residues_to_new_chain(structure: Structure.Structure, chain_id: str, positions: list[int], new_chain_id: str = "X") -> Structure.Structure:
    """
...
def remove_chain(structure: Structure.Structure, chain_id: str) -> Structure.Structure:
    for model in structure:
...
def get_chains_from_structure(structure: Structure.Structure) -> list[str]:
    """
...
def extract_sequences_from_structure(structure: Structure.Structure) -> dict[str, str]:
    ppb = PPBuilder()
    sequences = {}
...
# ==============================================================================
# // Additional utility functions that take files as input and use Biopython
# ==============================================================================


def slice_out_residues(pdb_file, chain_id=None, start_idx=1, end_idx=30):
    pdb_file = Path(pdb_file)
    chains = get_chains_from_structure(pdb_file)
    if chain_id is None:
        chain_id = chains[0]
    parser = PDB.PDBParser(QUIET=True)
    structure = parser.get_structure("protein", pdb_file)
...
def extract_chain_sequence_from_pdb_resnames(input_file, chain_id="B"):
...
def get_chain_center_of_mass_from_pdb(pdb_file, chain_id):
    """
...
def find_intrachain_disulfides(
    pdb_file: str | Path,
    distance_threshold: float = 2.6,
) -> dict[str, list[tuple[int, int]]]:
    """Detect intrachain disulfides geometrically (no reliance on SSBOND records).

    Args:
        pdb_file: Path to PDB.
        distance_threshold: Max SG–SG distance (Å) to consider a disulfide.

    Returns:
        Mapping from chain ID to list of (resseq_i, resseq_j) tuples where i<j.
    """
    ...
    def select_sg(residue):
        ...
        if not sg_atoms:
            return None
        # Prefer highest occupancy; fall back to first
        sg_atoms.sort(key=lambda a: a.get_occupancy() if a.get_occupancy() is not None else 0.0, reverse=True)
        return sg_atoms[0]

    ss_by_chain: dict[str, list[tuple[int, int]]] = defaultdict(list)

    for model in structure:  # type: ignore
        for chain in model:
            # Collect free cysteine SG atoms
            cysteines = []
            for residue in chain:
                if residue.get_id()[0] != " ":
                ...
                if sg_atom is None:
                    continue
                cysteines.append((residue, sg_atom))

            # Check all pairs within chain
            n = len(cysteines)
            for i in range(n):
                res_i, sg_i = cysteines[i]
                for j in range(i + 1, n):
                    res_j, sg_j = cysteines[j]
                    dist = np.linalg.norm(sg_i.get_coord() - sg_j.get_coord())
                    if dist <= distance_threshold:
                        resseq_i = res_i.get_id()[1]
                        resseq_j = res_j.get_id()[1]
                        ss_by_chain[chain.id].append((resseq_i, resseq_j))
